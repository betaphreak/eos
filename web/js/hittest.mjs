"use strict";
// What's under the cursor: province and plot hit-testing, across the map's east-west wrap copies.
// Split out of panel.mjs — pure geometry over the camera and the loaded grids, with no DOM and no
// rail state, so both the panel (hover/select) and bandcaption.mjs (viewport focus) can ask without
// pulling in the whole panel module.
import { P, cam, VIEW, pxr, pyr, px, py, worldW, provBoxHas } from "./core.mjs";
import { atLeast, BAND } from "./bands.mjs";
import { bonusIconRect } from "./bonusicons.mjs";

// The on-screen world copies the cursor could fall in: the map wraps east-west, so each test shifts
// the cursor into a copy's primary space (mx − m·period) and tries again.
function wrapCopies() {
  const period = worldW();
  const L = cam.x + cam.k * VIEW.dx;
  return {
    period,
    mMin: period > 0 ? Math.floor((0 - L) / period) : 0,
    mMax: period > 0 ? Math.floor((VIEW.w - L) / period) : 0,
  };
}
...
export function provinceAt(mx, my) {
  const { period, mMin, mMax } = wrapCopies();
