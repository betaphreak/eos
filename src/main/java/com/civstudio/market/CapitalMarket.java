...
public class CapitalMarket extends Market {
	 ...
	 */
	public CapitalMarket(Settlement colony) {
		super("Capital", colony);
	...
	public void clear() {
		mktGoodVol = 0;
		Collections.shuffle(buyOffers, colony.getRng().getRandom());
		for (BuyOffer buyOffer : buyOffers) {
			if (supply == 0)
				return;
			SellOffer picked = null;
			while (picked == null) {
				double winner = colony.getRng().uniform();
				double val = 0;
	...
	public String toString() {
		return String.format(
				"%s [avgPrice=%.3f lastVol=%.1f buys=%d sells=%d]",
				name, avgPrice, mktGoodVol, buyOffers.size(), sellOffers.size());
	}
