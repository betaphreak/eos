...
- **MCP tool surface** — the sim as typed tools for an LLM. A Spring AI 2.0 MCP server co-hosted in `civstudio-server` (`spring-ai-starter-mcp-server-webmvc`, Streamable HTTP at `/mcp`) exposes read-only tools/resources over the live `SessionHost` (`com.civstudio.server.mcp`: `list_sessions`/`get_snapshot`/`get_person`/`get_command_log` + `civstudio://session/{id}/snapshot|events`). Write tools (admin-gated) and the engine-only Phase-1 calibration harness remain to build. `docs/mcp-server.md`.
