...
**Web visualization** — `web/` is a dependency-free static site (`index.html` + `app.js` + generated `data.js`) that **replays** a run's caravan output over the real EU4 terrain map; it is a read-only consumer of `output/<seed>/` and never touches the engine. Rebuild with `node web/build.mjs [seed]`. Details in [`web/README.md`](web/README.md).
