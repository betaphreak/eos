...
- **Trade goods** — per-province EU4 resource (`Province.tradeGood` + `TradeGood`/`TradeGoodClass` records + `WorldMap.provincesOfTradeGood`), stamped from Anbennar history by `ProvinceHistoryExporter` / `TradeGoodExporter`; reference data, nothing consumes it yet. Per-province (cf. the per-plot `Bonus`). `docs/trade-goods.md`.
