...
- **Laborers** (`agent/laborer/Laborer.java`) earn wages, decide consumption vs. savings (target-savings model sensitive to the real interest rate), buy Necessity and Enjoyment, eat one unit of Necessity per step, and `die()` if they can't eat. Each laborer is a **household** and names itself in its constructor from `economy.getNames()` (see `name/` below): a `Person head` with a unique dynasty surname plus a male given name; the death log reports `head.fullName()`.
