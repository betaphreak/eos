    ...
    async bulkCreate(ctx) {
        try {
            ...
            if (!Array.isArray(data)) {
                return ctx.badRequest('Expected an array of country objects in the "data" wrapper.');
            }
            ...
            for (const item of data) {
                ...
                const record = await strapi.documents('api::country.country').create({
                    data: item,
                });
            ...
            return ctx.send({
                message: `Successfully created ${createdRecords.length} countries.`,
                data: createdRecords
            });
