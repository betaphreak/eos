...
export default {
  ...
  async buildings() {
    return (await all(uid('building'), { sort: ['key:asc'], fields: ['key', 'name', 'pedia', 'category', 'artDefineTag', 'button', 'cost', 'autoBuild', 'kind', 'flavors', 'replacedBy'], populate: { prereqTech: { fields: ['key'] }, obsoleteTech: { fields: ['key'] }, andTechs: { fields: ['key'] } } }))
      .map((r) => clean({ id: r.key, name: r.name, pedia: r.pedia, category: r.category, prereqTech: r.prereqTech?.key, andTechs: ks(r.andTechs), artDefineTag: r.artDefineTag, button: r.button, cost: str(r.cost), obsoleteTech: r.obsoleteTech?.key, autoBuild: r.autoBuild, kind: r.kind, flavors: r.flavors, replacedBy: r.replacedBy }));
  },
  async recipes() {
  ...
  async housing() {
    return (await all(uid('housing'), { sort: ['key:asc'], fields: ['key', 'name', 'cost', 'authoredCost', 'prereqPopulation', 'freshWater', 'autoBuild', 'health', 'happiness', 'yieldChanges', 'commerceChanges'], populate: { prereqTech: { fields: ['key'] }, obsoleteTech: { fields: ['key'] }, obsoletesToBuilding: { fields: ['key'] }, bonus: { fields: ['key'] }, prereqBonuses: { fields: ['key'] }, prereqBuildings: { fields: ['key'] }, prereqOrBuildings: { fields: ['key'] }, replacements: { fields: ['key'] }, prereqOrFeatures: { fields: ['key'] }, prereqOrTerrains: { fields: ['key'] } } }))
      .map((r) => clean({ type: r.key, name: r.name, cost: r.cost, authoredCost: r.authoredCost, prereqTech: r.prereqTech?.key, obsoleteTech: r.obsoleteTech?.key, obsoletesToBuilding: r.obsoletesToBuilding?.key, prereqPopulation: r.prereqPopulation, freshWater: r.freshWater, autoBuild: r.autoBuild, bonus: r.bonus?.key, prereqBonuses: ks(r.prereqBonuses), prereqBuildings: ks(r.prereqBuildings), prereqOrBuildings: ks(r.prereqOrBuildings), prereqOrFeatures: ks(r.prereqOrFeatures), prereqOrTerrains: ks(r.prereqOrTerrains), replacements: ks(r.replacements), health: r.health, happiness: r.happiness, yieldChanges: r.yieldChanges, commerceChanges: r.commerceChanges }));
  },
