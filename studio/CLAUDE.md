...
- **UID vs folder:** a content-type UID is `api::<apiFolder>.<contentTypeFolder>`; Strapi requires the
  *content-type* folder to equal `singularName` but the *api* folder can differ. Two single types use
  this (`api::era-modifiers.era-modifier`, `api::region-earth-map.region-name`) because a live IntelliJ
  VFS lock blocked renaming the api folders — rename them to match when the lock is clear.
