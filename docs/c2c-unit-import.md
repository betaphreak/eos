...
| 2 | **Gate** | **Identical to buildings.** A unit is in-scope iff it has a `<PrereqTech>` **and its whole tech-prereq expression resolves within the kept techs** — the primary `<PrereqTech>` **and** every `<TechTypes>` AND-entry must name a tech surviving into `techs.json` (the Prehistoric→Atomic horizon capped at `TECH_INFORMATION_LIFESTYLE`). Reuse the exact kept-tech set (read from the baked `techs.json`) and the `CAP_TECH` handling from `BuildingInfoExporter`. |
